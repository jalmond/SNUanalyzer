...
void SKTreeMakerHNDiLep::ExecuteEvents()throw( SNUError ){
  ...
  if(muColl.size()==2) {
    if(!SameCharge(muColl)) throw SNUError( "Not Lepton Event",  SNUError::SkipEvent );
  }
  else if(elColl.size()==2) {
    if(!SameCharge(elColl)) throw SNUError( "Not Lepton Event",  SNUError::SkipEvent );
  }
  else{
    if(muColl.size()==1 && elColl.size()==1 ) {
      if(elColl[0].Charge() != muColl[0].Charge()) throw SNUError( "Not Lepton Event",  SNUError::SkipEvent );
    }
  }
...
void SKTreeMakerHNDiLep::EndCycle()throw( SNUError ){
  
  Message("In EndCycle" , INFO);
}


void SKTreeMakerHNDiLep::BeginCycle() throw( SNUError ){
...
void SKTreeMakerHNDiLep::BeginEvent( )throw( SNUError ){
...
void SKTreeMakerHNDiLep::ClearOutputVectors() throw (SNUError){
  //
