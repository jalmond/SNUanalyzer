...
void HNEMu::InitialiseAnalysis() throw( SNUError ) {
  ...
  if(k_flags.size() > 0){
    ...
    if(k_flags[0] == "CutFlow")  functionality = HNEMu::CUTFLOW;
    else if(k_flags[0] == "OPT")  functionality = HNEMu::OPT;
    else functionality = HNEMu::ANALYSIS;
  }


  
  if(  functionality == HNEMu::ANALYSIS){
    ...
    if(!k_isdata || (k_channel=="DoubleMuon" || k_channel=="SingleMuon")){
      ...
      labels.push_back("Muon_inclusive");
      labels.push_back("DiMuon_inclusive");
      labels.push_back("DiMuon_inclusive_8TeV");
	...
	MakeCleverHistograms(llhist, label+"SSLL_LowMass_8TeV");
      ...
      else       MakeCleverHistograms(llhist, label+"SSLL_HighMass_SR2");
...
void HNEMu::ExecuteEvents()throw( SNUError ){
  ...
  if(functionality==HNEMu::CUTFLOW){
    DoCutFlow(1.); return;
  }
  ...
  if(functionality==HNEMu::ANALYSIS){
    ...
    if(_m_channel)RunMM(0,"Muon_inclusive",      muons,muons_veto,electrons_veto,alljets,   jets_all20, fatjetcoll_updated     ,  tchanjets,  mm_weight, triggerlist_DiMuon_singleLep ,25.      , 5.);    
    if(_mm_channel)RunMM(0,"DiMuon_inclusive_fake",      muons_fake,muons_veto,electrons_veto,alljets,   jets_all20, fatjetcoll_updated     ,  tchanjets,  mm_weight, triggerlist_DiMuon ,20.      , 10.);    
    
    if(_mm_channel)RunMM(0,"DiMuon_inclusive_8TeV",      muons8TeV,muons_veto,electrons_veto,alljets,   jets_all20, fatjetcoll_updated     ,  tchanjets,  mm_weight, triggerlist_DiMuon ,20.      , 10.);
    ...
    if(fatjetcoll_updated.size() ==0){
      ...
      if(_mm_channel)RunMM(1,"DiMuon_j25_TMP",      muons,muons_veto,electrons_veto,alljets,   jets_25, fatjetcoll_updated     ,  tchanjets,  mm_weight, triggerlist_DiMuon ,20.      , 10.);
      if(_mm_channel)RunMM(1,"DiMuon_j30_TMP",      muons,muons_veto,electrons_veto,alljets,   jets_30, fatjetcoll_updated     ,  tchanjets,  mm_weight, triggerlist_DiMuon ,20.      , 10.);
...
float HNEMu::EEWeight(std::vector<snu::KElectron> electrons,TString id){
...
float HNEMu::EMWeight(std::vector<snu::KElectron> electrons,std::vector<snu::KMuon> muons,TString idel, TString idmu){
...
float HNEMu::MMWeight(std::vector<snu::KMuon> muons, TString id, bool passtrig){
...
void HNEMu::RunMM(int mode, TString label, vector<snu::KMuon> muons, vector<snu::KMuon> muons_veto, vector<snu::KElectron> electrons, vector<snu::KJet> alljets, vector<snu::KJet> jets, vector<snu::KFatJet> fatjets, vector<snu::KJet> tjets,float mm_weight ,vector<TString> mm_trig, float pt1, float pt2){
...
void HNEMu::RunEE(TString label, vector<snu::KElectron> electrons, vector<snu::KElectron> electrons_veto, vector<snu::KMuon> muons, vector<snu::KJet> alljets, vector<snu::KJet> jets, vector<snu::KFatJet> fatjets, vector<snu::KJet> tjets,float ee_weight ,vector<TString> ee_trig, float pt1, float pt2){
...
void HNEMu::RunLL(int mode,TString channel , TString label, vector<snu::KMuon> muons, vector<snu::KMuon> muons_veto, vector<snu::KElectron> electrons,vector<snu::KElectron> electrons_veto, vector<snu::KJet> alljets, vector<snu::KJet> jets, vector<snu::KFatJet> fatjets, vector<snu::KJet> tjets,float ll_weight ,vector<TString> ll_trig, float pt1, float pt2){
  ...
  if(cut1_dilep){
	...
	if(mode<2){
	  if(jets.size()==1 ){
	    ...
	    if( (GetDiLepMass(muons) < 100)){
	      ...
	      if(l2j < 80 && l1j < 100 && llj < 150 &&GetDiLepMass(muons) > 15){
	    ...
	    // BIN 2 is 1 jet region
	      
	    //bool HNEMu::PassLowMassBin2(float pt1max, float pt2max, float mlljmax, float ml1jmax, float ml2jmax, float metmax, float mllmin, float mllmax, float pt2min, std::vector<snu::KMuon> muons,  stxd::vector<snu::KJet> jets, std::vector
	    //https://docs.google.com/spreadsheets/d/1PGH5-yDB3JzXVzHwIvS1qpg1RV6zbLVVvLe3fxTDlfQ/edit#gid=0
	  ...
	  //bool HNEMu::PassLowMassBin1(float pt1max, float pt2max,float mjjmax, float mlljjmin,  float mlljjmax, float ml1jjmax, float ml2jjmax, float metmax, float mllmin, float mll max, float pt2min, std::vector<snu::KMuon> muons,  std::vector<snu::KJet> jets, std::vector<snu::KFatJet> fjets, int nbjet){
	  ...
	  if(PassLowMassBin1(10000., 10000., 120.,80., 200.,10000., 100000., 30., 15., 100000, 15., muons,jets,fatjets,nbjet_m)){
	    ...
	    if(FillAll)FillCLHist(llhist, label+"SSLL_LowMass_8TeV", eventbase->GetEvent(), muons, electrons,jets, alljets,  fatjets,ll_weight);
	  }
	  ...
	  //bool HNEMu::PassHighMassBin1(float pt1min, float pt2min, float ml2jjmin, float ml2jjmax, float mlljjmin,  float mjjmin, float mjjmax, float met2st, std::vector<snu::KMuon> muons,  std::vector<snu::KJet> jets, std::vector<snu::KFatJet> fjets, int nbjet, float met2_st){
	...
	if(mode==2 && fatjets.size() > 0) {
	  ...
	  if(CheckSignalRegion(true,muons,electrons_veto , jets,fatjets, alljets,"LowMM", ll_weight, mode)){
	    ...
	    if(nbjet_m == 0){
	      ...
	      if(met < 80){
		if(FillAll){
		  if(jets.size() >1)FillCLHist(llhist, label+"SSLL_LowMass_SR", eventbase->GetEvent(), muons, electrons,jets, alljets,  fatjets,ll_weight);
		  if(jets.size()==1 )                  FillCLHist(llhist, label+"SSLL_LowMass_1Jet_SR", eventbase->GetEvent(), muons, electrons,jets, alljets,  fatjets,ll_weight);
		...
		if(met<40){
		  if(jets.size()> 1&& muons[1].Pt() > 10)FillEventCutFlow(4,"SSLL_LowMass_1Jet_SR_pt2_5",ll_weight,label);
		...
		if(muons[1].Pt()>15 && muons[1].Pt() < 50.){
		  if(muons[0].Pt() < 80){
		    if(met < 40)FillHist(label+"Truth_WZ_SS_LowMass_Zmass",GetVirtualMass(13,false,false), ll_weight*WeightByTrigger(ll_trig[0], TargetLumi), 0., 150., 150);
		  }
		}
	      }
		...
		if(muons[1].Pt()>15 && muons[1].Pt() < 50. && muons[0].Pt() < 80.&& mlljj < 250 && ml1jj < 200 && ml2jj< 150 && met < 40 && GetDiLepMass(electrons) < 100&& FillAll)FillCLHist(llhist, label+"SSLL_LowMass_SR2", eventbase->GetEvent(), muons, electrons,jets, alljets,  fatjets,ll_weight);
	      }
	    }
	    if(met > 100 || nbjet_m > 0)if(FillAll)FillCLHist(llhist, label+"SSLL_LowMass_CR", eventbase->GetEvent(), muons, electrons,jets, alljets,  fatjets,ll_weight);
	  }
	}	
	
	
	
	if((CheckSignalRegion(true,muons,electrons_veto , jets, fatjets,alljets,"HighMM", ll_weight,mode)) ){
	  if( (mode==0 && jets.size() > 1)  || (mode==1 &&jets.size() > 1 && fatjets.size() ==0) || (mode==2 && fatjets.size()>0)){
		...
		if(FillAll)FillCLHist(llhist, label+"SSLL_HighMass_SR", eventbase->GetEvent(), muons, electrons,jets, alljets,  fatjets,ll_weight);
		...
		if(met2_st < 7){
		  if(mode==1) {
		    if(fatjets.size() ==0){
		      ...
		      if(muons[0].Pt() >110 && muons[1].Pt() > 40 && mlljj > 800, mjj > 30 && mjj < 150)               if(FillAll)FillCLHist(llhist, label+"SSLL_HighMass_SR2", eventbase->GetEvent(), muons, electrons,jets, alljets,  fatjets,ll_weight);
		    }
		  }
		  if(mode==2) {
		    if(fatjets.size() >0){
		      ...
		      if(muons[0].Pt() >140 && muons[1].Pt() > 70 && mllfj > 640, mjj > 30 && mjj < 150)               if(FillAll)FillCLHist(llhist, label+"SSLL_HighMass_SR2", eventbase->GetEvent(), muons, electrons,jets, alljets,  fatjets,ll_weight);
		      
		    }
		  }
		  
		  FillHist(label+"Truth_WZ_SS_HighMass_Zmass",GetVirtualMass(13,false,false), ll_weight*WeightByTrigger(ll_trig[0], TargetLumi), 0., 150., 150);
		}
	      }
	      if(mode ==0 ){
		if(met2_st > 20 || nbjet_m > 0)if(FillAll)FillCLHist(llhist, label+"SSLL_HighMass_CR", eventbase->GetEvent(), muons, electrons,jets, alljets,  fatjets,ll_weight);
	      }
	      if(mode==1){
		if(fatjets.size() ==0){
		  if(met2_st > 20 || nbjet_m > 0)if(FillAll)FillCLHist(llhist, label+"SSLL_HighMass_CR", eventbase->GetEvent(), muons, electrons,jets, alljets,  fatjets,ll_weight);
...
bool HNEMu::PassLowMassBin2(float pt1max, float pt2max, float mlljmax, float ml1jmax, float ml2jmax, float metmax, float mllmin, float mllmax, float pt2min, std::vector<snu::KMuon> muons,  std::vector<snu::KJet> jets, std::vector<snu::KFatJet> fjets, int nbjet){
...
bool HNEMu::PassHighMassBin1Window(float window, float mass, float pt1min, float pt2min, float ml2jjmin, float ml2jjmax, float mlljjmin,  float mjjmin, float mjjmax, float met2st, std::vector<snu::KMuon> muons,  std::vector<snu::KJet> jets, std::vector<snu::KFatJet> fjets, int nbjet, float met2_st){
...
bool HNEMu::PassHighMassBin1(float pt1min, float pt2min, float ml2jjmin, float ml2jjmax, float mlljjmin,  float mjjmin, float mjjmax, float met2st, std::vector<snu::KMuon> muons,  std::vector<snu::KJet> jets, std::vector<snu::KFatJet> fjets, int nbjet, float met2_st, bool debug){
...
bool HNEMu::PassHighMassBin2Window(float mass,float pt1max, float pt2max, float mllfjmin,  float met2st, std::vector<snu::KMuon> muons,  std::vector<snu::KJet> jets, std::vector<snu::KFatJet> fjets, int nbjet, float met2_st){
...
bool HNEMu::PassHighMassBin2(float pt1max, float pt2max, float mllfjmin,  float met2st, std::vector<snu::KMuon> muons,  std::vector<snu::KJet> jets, std::vector<snu::KFatJet> fjets, int nbjet, float met2_st){
...
bool HNEMu::PassLowMassBin1(float pt1max, float pt2max,float mjjmax, float mlljjmin,  float mlljjmax, float ml1jjmax, float ml2jjmax, float metmax, float mllmin, float mllmax, float pt2min, std::vector<snu::KMuon> muons,  std::vector<snu::KJet> jets, std::vector<snu::KFatJet> fjets, int nbjet){
...
float HNEMu::WeightCFEvent(std::vector<snu::KElectron> electrons, bool runchargeflip){
...
void HNEMu::FillEventCutFlow(int cf,TString cut,  float wt, TString label){
...
void  HNEMu::FillEfficiency(TString lab, vector<snu::KJet> jets,  vector<snu::KFatJet> fatjets,float w){
...
vector<int>   HNEMu::FillTruthPlots(std::vector<snu::KJet> jets, std::vector<snu::KMuon> muons, TString label){
...
int HNEMu::MatchedLeptonJets(std::vector<snu::KJet> jets,std::vector<snu::KFatJet> fatjets, std::vector<snu::KMuon> muons, TString label, vector<int> i_jets){
...
void HNEMu::GetTriggEfficiency(float pt_cut, TString label){
...
void HNEMu::FillTriggerEfficiency(TString label2, TString cut, float weight, TString label, std::vector<TString> list){
...
bool HNEMu::CheckSignalRegion( bool isss,  std::vector<snu::KMuon> muons, std::vector<snu::KElectron> el,  std::vector<snu::KJet> jets,  std::vector<snu::KFatJet> fjets,   std::vector<snu::KJet> alljets, TString name, float w, int mode){
...
bool HNEMu::OptMassCheckSignalRegion(std::vector<snu::KElectron> electrons, std::vector<snu::KJet> jets, float evmet, bool runchargeflip , std::vector<float> cuts , TString additional_option){
  
  return OptMassCheckSignalRegion(electrons, jets, 1, evmet, runchargeflip, cuts, additional_option);
}

bool HNEMu::OptMassCheckSignalRegion(std::vector<snu::KElectron> electrons, std::vector<snu::KJet> jets, int btagwp,  float evmet, bool runchargeflip , std::vector<float> cuts, TString additional_option){
...
bool HNEMu::LowMassCheckSignalRegion(  std::vector<snu::KElectron> electrons, std::vector<snu::KJet> jets, bool runchargeflip){
...
bool HNEMu::MidMassCheckSignalRegion(  std::vector<snu::KElectron> electrons, std::vector<snu::KJet> jets, bool runchargeflip){
...
bool HNEMu::HighMassCheckSignalRegion(  std::vector<snu::KElectron> electrons, std::vector<snu::KJet> jets, bool runchargeflip){
...
void HNEMu::counter(TString cut, float w){
