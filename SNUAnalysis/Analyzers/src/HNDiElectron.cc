...
void HNDiElectron::ExecuteEvents()throw( SNUError ){
  ...
  if(functionality==HNDiElectron::ANALYSIS){
    ...
    std::vector<TString> triggerlist_DiElectron, triggerlist_DiMuon;
    ...
    if(_ee_channel)RunEE(0,"DiElectron_inclusive",   electrons, electrons_veto, muons_veto,alljets,   jets_all20, fatjetcoll_updated     ,  tchanjets,  ee_weight, triggerlist_DiMuon ,25.      , 15.);    
    if(_e_channel)RunEE(0,"DiElectron_inclusive" , electrons,   electrons_veto, muons_veto,alljets,   jets_all20, fatjetcoll_updated     ,  tchanjets,  ee_weight, triggerlist_DiMuon_singleLep ,30.      , 10.);  

    if(fatjetcoll_updated.size() ==0) {
      if(_mm_channel)RunEE(1,"DiMuon_NoFAT",      electrons, electrons_veto,muons_veto, alljets,   jets_20, fatjetcoll_updated     ,  tchanjets,  ee_weight, triggerlist_DiMuon ,20.      , 10.);
    }
    else{
      if(_mm_channel)RunEE(2,"DiMuon_FAT",      electrons , electrons_veto,  muons_veto, alljets,   jets_20, fatjetcoll_updated     ,  tchanjets,  ee_weight, triggerlist_DiMuon ,20.      , 10.);
    }
...
void HNDiElectron::RunLL(int mode,TString channel , TString label, vector<snu::KMuon> muons, vector<snu::KMuon> muons_veto, vector<snu::KElectron> electrons,vector<snu::KElectron> electrons_veto, vector<snu::KJet> alljets, vector<snu::KJet> jets, vector<snu::KFatJet> fatjets, vector<snu::KJet> tjets,float ll_weight ,vector<TString> ll_trig, float pt1, float pt2){
  ...
  std::vector<snu::KMuon> muons_tight= GetMuons("MUON_HN_TIGHT");

  if(electrons.size() == 2 && muons_tight.size() ==1){
    if(!SameCharge(electrons)){
      if(met > 40){
	if(nbjet_m==0&&cut2_trig){
	  if(muons_tight.at(0).Pt() > 10 && electrons[0].Pt() > 25 && electrons[1].Pt() > 15){
	    float dphi_1 = fabs(TVector2::Phi_mpi_pi(muons_tight.at(0).Phi()- eventbase->GetEvent().METPhi(snu::KEvent::pfmet)));
	    float MT_1 = sqrt(2.* muons_tight.at(0).Et()*eventbase->GetEvent().PFMET() * (1 - cos( dphi_1)));
	    snu::KParticle m2 = electrons[0]+electrons[1];
	    
	    snu::KParticle m3 = electrons[0]+electrons[1]+muons_tight[0];
  ...
  if(label.Contains("DiElectron_inclusive")) {
    std::vector<snu::KElectron> electrons_loose= GetElectrons(_e_looseid,false);
    if(SameCharge(electrons_loose)){
      if( (PassID(electrons_loose[0], _e_tightid) && ! PassID(electrons_loose[1],_e_tightid)) ||  (!PassID(electrons_loose[0],_e_tightid) && PassID(electrons_loose[1],_e_tightid))){
	
	if(FillAll)FillCLHist(sighist_ee, label+"SSLL_TL", eventbase->GetEvent(), muons, electrons_loose,jets, alljets, fatjets,  ll_weight);
      }
    }
  }
  if(cut1_dilep){
	...
	if(mode<2){
	  if(jets.size()==1 ){
	    ...
	    //bool HNDiElectron::PassLowMassBin2(float pt1max, float pt2max, float mlljmax, float ml1jmax, float ml2jmax, float metmax, float mllmin, float mllmax, float pt2min, std::vector<snu::KMuon> muons,  stxd::vector<snu::KJet> jets, std::vector
	  ...
	  //bool HNDiElectron::PassLowMassBin1(float pt1max, float pt2max,float mjjmax, float mlljjmin,  float mlljjmax, float ml1jjmax, float ml2jjmax, float metmax, float mllmin, float mll max, float pt2min, std::vector<snu::KMuon> muons,  std::vector<snu::KJet> jets, std::vector<snu::KFatJet> fjets, int nbjet){
	  ...
	  //bool HNDiElectron::PassHighMassBin1(float pt1min, float pt2min, float ml2jjmin, float ml2jjmax, float mlljjmin,  float mjjmin, float mjjmax, float met2st, std::vector<snu::KMuon> muons,  std::vector<snu::KJet> jets, std::vector<snu::KFatJet> fjets, int nbjet, float met2_st){
...
bool HNDiElectron::PassLowMassBin2(float pt1max, float pt2max, float mlljmax, float ml1jmax, float ml2jmax, float metmax, float mllmin, float mllmax, float pt2min, std::vector<snu::KMuon> muons,  std::vector<snu::KJet> jets, std::vector<snu::KFatJet> fjets, int nbjet){
...
bool HNDiElectron::PassHighMassBin1Window(float window, float mass, float pt1min, float pt2min, float ml2jjmin, float ml2jjmax, float mlljjmin,  float mjjmin, float mjjmax, float met2st, std::vector<snu::KMuon> muons,  std::vector<snu::KJet> jets, std::vector<snu::KFatJet> fjets, int nbjet, float met2_st){
...
bool HNDiElectron::PassHighMassBin1(float pt1min, float pt2min, float ml2jjmin, float ml2jjmax, float mlljjmin,  float mjjmin, float mjjmax, float met2st, std::vector<snu::KMuon> muons,  std::vector<snu::KJet> jets, std::vector<snu::KFatJet> fjets, int nbjet, float met2_st, bool debug){
...
bool HNDiElectron::PassHighMassBin2Window(float mass,float pt1max, float pt2max, float mllfjmin,  float met2st, std::vector<snu::KMuon> muons,  std::vector<snu::KJet> jets, std::vector<snu::KFatJet> fjets, int nbjet, float met2_st){
...
bool HNDiElectron::PassHighMassBin2(float pt1max, float pt2max, float mllfjmin,  float met2st, std::vector<snu::KMuon> muons,  std::vector<snu::KJet> jets, std::vector<snu::KFatJet> fjets, int nbjet, float met2_st){
...
bool HNDiElectron::PassLowMassBin1(float pt1max, float pt2max,float mjjmax, float mlljjmin,  float mlljjmax, float ml1jjmax, float ml2jjmax, float metmax, float mllmin, float mllmax, float pt2min, std::vector<snu::KMuon> muons,  std::vector<snu::KJet> jets, std::vector<snu::KFatJet> fjets, int nbjet){
...
void  HNDiElectron::FillEfficiency(TString lab, vector<snu::KJet> jets,  vector<snu::KFatJet> fatjets,float w){
  ...
  for(unsigned int iid=0; iid < muonIDs.size(); iid++){
    ...
    std::vector<snu::KElectron> muons_eff=GetElectrons(elid,false);
	...
	if(muons_eff[1].Pt() > 15){
      ...
      else if(muonIDs[iid] == _e_tightid) {
...
vector<int>   HNDiElectron::FillTruthPlots(std::vector<snu::KJet> jets, std::vector<snu::KMuon> muons, TString label){
...
int HNDiElectron::MatchedLeptonJets(std::vector<snu::KJet> jets,std::vector<snu::KFatJet> fatjets, std::vector<snu::KMuon> muons, TString label, vector<int> i_jets){
...
bool HNDiElectron::CheckSignalRegion( bool isss,  std::vector<snu::KMuon> muons, std::vector<snu::KElectron> el,  std::vector<snu::KJet> jets,  std::vector<snu::KFatJet> fjets,   std::vector<snu::KJet> alljets, TString name, float w, int mode){
